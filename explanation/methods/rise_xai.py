...
class RISE(nn.Module):
    def __init__(self, model, n_masks=10000, p1=0.1, input_size=(224, 224), initial_mask_size=(7, 7), n_batch=128, mask_path=None, adv_image=None):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        if(self.adv_image==None):
            # shape (n_masks, 3, H, W)
            masked_x = torch.mul(self.masks, x.to('cpu').data)
        else:
            masked_x = torch.mul(self.masks, x.to('cpu').data) + torch.mul(1.0 - self.masks, self.adv_image.to('cpu').data)
...
def explain(inference_image,visualize_image,label,model,adv_mask=False,visualize=True):
    ...
    if (adv_mask):
        ...
        wrapped_model = RISE(model, n_masks=4000, input_size=input_size, adv_image=mask.to("cuda"))
    else:
        #Compute the explanation
