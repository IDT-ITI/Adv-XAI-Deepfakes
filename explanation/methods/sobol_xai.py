...
from sobol_attribution_method.torch_perturbations import adversarial
...
def explain(inference_image,visualize_image,label,model,adv_mask=False,visualize=True):
    ...
    if (adv_mask):
        ...
        explainer = SobolAttributionMethod(model, grid_size=8, nb_design=32, batch_size=128, perturbation_function=adversarial, adv_image=mask.to("cuda"))
    else:
        #Compute the explanation
