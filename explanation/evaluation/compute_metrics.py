...
def computeDiscScores(i_img,evaluation,metrics,segments,ranked_segments,label,num_of_segments=3):
    ...
    for i in range(num_of_segments):
        ...
        a_img = evaluation.generateAdversarialImage(mask)
    ...
    #Return the disc plus scores, the adversarially attacked images on top 1, top 1-2, top 1-2-3
    return [disc_plus,a_imgs]
...
def computeLIMEMetrics(model, i_img, v_img, i_trans, best_label, true_label,num_of_segments=3):
    ...
    if(len(ranked_segments)>=num_of_segments):
        ...
        disc_plus, a_imgs = computeDiscScores(i_img, evaluation, metrics, segments, ranked_segments, best_label)
...
def computeLIMEadvMetrics(model, i_img, v_img, i_trans, best_label, true_label, num_of_segments=3):
    #Custom segmentation function passed in LIME to segment the image
    def custom_seg(image):
        return slic(image, n_segments=50) - 1
    ...
    #Call the explanation method to produce the heatmap(binary mask) for the model's predicted label for the image
    heatmap, segments, exp = LIME(v_img.permute(1, 2, 0).numpy(), i_trans, best_label, model, custom_seg=custom_seg, adv_mask=True, visualize=False)
...
def computeSHAPadvMetrics(model, i_img, v_img, best_label, true_label):
    ...
    #Call the explanation method to produce the heatmap for the model's predicted label for the image
    heatmap = SHAP(i_img, v_img.permute(1, 2, 0).numpy(), best_label, model, adv_mask=True, visualize=False)
...
def computeSOBOLadvMetrics(model, i_img, v_img, best_label, true_label):
    ...
    #Call the explanation method to produce the heatmap for the model's predicted label for the image
    heatmap = SOBOL(i_img, v_img, best_label, model, adv_mask=True, visualize=False)
...
def computeRISEadvMetrics(model, i_img, v_img, best_label, true_label):
    ...
    #Call the explanation method to produce the heatmap for the model's predicted label for the image
    heatmap = RISE(i_img, v_img.unsqueeze(0), best_label, model, adv_mask=True, visualize=False)
...
def computeExplanationMetrics(model,ds,ds_vis,i_trans,evaluation_explanation_methods):
    ...
    for idx in range(start_index, len(ds.df)):
        ...
        if(evaluation_explanation_methods=="All"):
            ...
            scores_lime_adv = computeLIMEadvMetrics(model, frame, visualize_frame, i_trans, best_label, true_label)
            scores_shap = computeSHAPMetrics(model, frame, visualize_frame, best_label, true_label)
            scores_shap_adv=computeSHAPadvMetrics(model,frame,visualize_frame,best_label,true_label)
            scores_sobol = computeSOBOLMetrics(model, frame, visualize_frame, best_label, true_label)
            scores_sobol_adv = computeSOBOLadvMetrics(model, frame, visualize_frame, best_label, true_label)
            scores_rise = computeRISEMetrics(model, frame, visualize_frame, best_label, true_label)
            scores_rise_adv = computeRISEadvMetrics(model, frame, visualize_frame, best_label, true_label)

            # Shift the scores based on the pos index, so the mean of the scores are computed for examples of the same category
            scores_original = 12 * [np.nan] + [acc] + (3 * pos) * [np.nan] + [acc] + (11 - (3 * pos)) * [np.nan]
            scores_lime = (3 * pos) * [np.nan] + scores_lime[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_lime[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_lime_adv = (3 * pos) * [np.nan] + scores_lime_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_lime_adv[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_shap = (3 * pos) * [np.nan] + scores_shap[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_shap[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_shap_adv = (3 * pos) * [np.nan] + scores_shap_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_shap_adv[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_sobol = (3 * pos) * [np.nan] + scores_sobol[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_sobol[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_sobol_adv = (3 * pos) * [np.nan] + scores_sobol_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_sobol_adv[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_rise = (3 * pos) * [np.nan] + scores_rise[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_rise[-3:] + (9 - (3 * pos)) * [np.nan]
            scores_rise_adv = (3 * pos) * [np.nan] + scores_rise_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_rise_adv[-3:] + (9 - (3 * pos)) * [np.nan]

            # Accumulate the scores
            scores_img = np.array((scores_original, scores_lime, scores_lime_adv, scores_shap, scores_shap_adv, scores_rise, scores_rise_adv, scores_sobol, scores_sobol_adv))
        ...
        elif (evaluation_explanation_methods == "LIME_adv"):
            scores_lime_adv = computeLIMEadvMetrics(model, frame, visualize_frame, i_trans, best_label, true_label)

            scores_original = 12 * [np.nan] + [acc] + (3 * pos) * [np.nan] + [acc] + (11 - (3 * pos)) * [np.nan]
            scores_lime_adv = (3 * pos) * [np.nan] + scores_lime_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_lime_adv[-3:] + (9 - (3 * pos)) * [np.nan]

            scores_img = np.array((scores_original, scores_lime_adv))
            scores.append(scores_img)
        ...
        elif (evaluation_explanation_methods == "SHAP_adv"):
            scores_shap_adv = computeSHAPadvMetrics(model, frame, visualize_frame, best_label, true_label)

            scores_original = 12 * [np.nan] + [acc] + (3 * pos) * [np.nan] + [acc] + (11 - (3 * pos)) * [np.nan]
            scores_shap_adv = (3 * pos) * [np.nan] + scores_shap_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_shap_adv[-3:] + (9 - (3 * pos)) * [np.nan]

            scores_img = np.array((scores_original, scores_shap_adv))
            scores.append(scores_img)
        ...
        elif (evaluation_explanation_methods == "SOBOL_adv"):
            scores_sobol_adv = computeSOBOLadvMetrics(model, frame, visualize_frame, best_label, true_label)

            scores_original = 12 * [np.nan] + [acc] + (3 * pos) * [np.nan] + [acc] + (11 - (3 * pos)) * [np.nan]
            scores_sobol_adv = (3 * pos) * [np.nan] + scores_sobol_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_sobol_adv[-3:] + (9 - (3 * pos)) * [np.nan]

            scores_img = np.array((scores_original, scores_sobol_adv))
            scores.append(scores_img)
        ...
        elif (evaluation_explanation_methods == "RISE_adv"):
            scores_rise_adv = computeRISEadvMetrics(model, frame, visualize_frame, best_label, true_label)

            scores_original = 12 * [np.nan] + [acc] + (3 * pos) * [np.nan] + [acc] + (11 - (3 * pos)) * [np.nan]
            scores_rise_adv = (3 * pos) * [np.nan] + scores_rise_adv[:-3] + (9 - (3 * pos)) * [np.nan] + [np.nan] + (3 * pos) * [np.nan] + scores_rise_adv[-3:] + (9 - (3 * pos)) * [np.nan]

            scores_img = np.array((scores_original, scores_rise_adv))
            scores.append(scores_img)
